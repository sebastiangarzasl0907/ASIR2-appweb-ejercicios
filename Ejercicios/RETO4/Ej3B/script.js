function cambiar() { 
    ...
    let v_np = document.getElementsByClassName("np")

    // EJ3B AGREGO LOS HIJOS DEL DIV CORRESPONDIENTE
    ...
    for(i = 0; i < v_np.length; i++) {
        v_np[i].style.backgroundColor = 'red'
    }
}
...
function agregar() {
    ...
    // EJ3B, CREAMOS EL CUARTO PARRAFO JS Y PHP
    var div_js = document.createElement("p")
    div_js.className = "np"
    div_js.innerHTML = 'Este es el cuarto parrafo'
    document.getElementById('nuevoparrafojs').appendChild(div_js)

    var div_php = document.createElement("p")
    div_php.className = "np"
    div_php.innerHTML = 'Este es el cuarto parrafo'
    document.getElementById('nuevoparrafophp').appendChild(div_php)
}
